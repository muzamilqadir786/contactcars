...
class Db:
    ...
    def insert_post(self, data, _source='contactcaes'):
        ...
        query = '''INSERT INTO 26March2015
                   ( _widgetName,_source,_resultNumber,_pageUrl,make,model,model_year,price,cc,km,`desc`,addedon)
                   VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'''
        self.cur.execute(query, ('contactcaes',_source,'0',data['href'],data['maker'], data['model'],data['year'],
                                data['price'],data['cc'],data['km'],data['desc'],''))
        self.db.commit()
        return True
...
if __name__ == '__main__':
    print "hi there!!"
    db = Db(host='localhost',user='root',passwd='',db='pee')
    data = {}
    data['href'] = 'nn'
    data['desc'] = 'بحالة ودهان المصنع وصيانات في التوكيل'
    data['cc'] = '00'
    data['km'] = '00'
    data['price'] = '00'
    data['year'] = '22'
